    ...
    >
      <div className="bg-surface-card w-full sm:max-w-lg rounded-t-3xl sm:rounded-2xl overflow-hidden shadow-2xl max-h-[90vh] flex flex-col">
          ...
          <div className="flex items-start justify-between gap-3">
            <div>
              <div className="flex items-center gap-2 mb-1">
                <h2 className="text-[16px] font-bold text-ui-text">{slip.lotteryName}</h2>
                <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${STATUS_STYLE[status] ?? "bg-surface-subtle text-ui-text-soft"}`}>
        ...
        <div className="overflow-y-auto flex-1">
          {/* Column header */}
          <div className="grid grid-cols-[1fr_auto_auto_auto] gap-2 px-5 py-2 bg-surface-subtle text-[11px] font-semibold text-ui-text-muted uppercase tracking-wide sticky top-0">
            ...
            {slip.items.map((item) => (
              <div
                ...
                className={[
                  ...
                  item.isWin === true  ? "bg-ui-status-success/5" : "",
                  item.isWin === false ? "bg-ui-status-error/5"   : "",
                ].join(" ")}
              >
                {/* เลข + ประเภท */}
                <div>
                  <span className="text-[18px] font-bold text-ui-text tabular-nums tracking-widest">{item.number}</span>
                  <div className="flex items-center gap-1.5 mt-0.5">
                    <span className="text-[11px] text-ui-text-soft">{item.betTypeLabel}</span>
                    {item.isWin === true && (
                      <span className="text-[10px] font-bold bg-ui-status-success/10 text-ui-status-success px-1.5 py-0.5 rounded-full">ถูก ✓</span>
                    )}
                    {item.isWin === false && (
                      <span className="text-[10px] font-bold bg-ui-status-error/10 text-ui-status-error px-1.5 py-0.5 rounded-full">ไม่ถูก</span>
                    )}
                  </div>
                </div>
                {/* ยอดแทง */}
                <span className="text-[13px] font-semibold text-ui-text tabular-nums text-right">
                  ฿{item.amount.toLocaleString("th-TH")}
                </span>
                {/* อัตรา */}
                <span className="text-[13px] text-ui-text-soft tabular-nums text-right">
                  ×{item.payRate}
                </span>
                {/* ชนะสูงสุด / จริง */}
                <span className={`text-[13px] font-semibold tabular-nums text-right ${item.isWin === true ? "text-ui-status-success" : "text-ui-text-muted"}`}>
                  {item.isWin === true
        ...
        {/* Footer summary */}
        <div className="px-5 py-4 border-t border-ui-border bg-surface-subtle flex-shrink-0 space-y-1.5">
          {slip.note && (
            <p className="text-[12px] text-ui-text-soft mb-2">หมายเหตุ: {slip.note}</p>
          )}
          <div className="flex justify-between items-center">
            <span className="text-[13px] text-ui-text-soft">{slip.itemCount} รายการ</span>
            <span className="text-[13px] text-ui-text-soft tabular-nums">
              ฿{slip.totalBetAmount.toLocaleString("en-US", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
            </span>
          </div>
          {slip.totalDiscountAmount > 0 && (
            <div className="flex justify-between items-center">
              <span className="text-[13px] text-ui-status-success">ส่วนลด</span>
              <span className="text-[13px] font-semibold text-ui-status-success tabular-nums">
                -฿{slip.totalDiscountAmount.toLocaleString("en-US", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
              </span>
            </div>
          )}
          <div className="flex justify-between items-center">
            <span className="text-[13px] font-bold text-ui-text">ยอดสุทธิ</span>
          ...
          {status === "won" && totalWin > 0 ? (
            <div className="flex justify-between items-center">
              <span className="text-[13px] text-ui-text-soft">ได้รับจริง</span>
              <span className="text-[16px] font-bold text-ui-status-success tabular-nums">+฿{totalWin.toLocaleString("th-TH")}</span>
            </div>
          ) : (
            <div className="flex justify-between items-center">
              <span className="text-[13px] text-ui-text-soft">ชนะสูงสุด</span>
              <span className="text-[14px] font-semibold text-ui-text-muted tabular-nums">฿{slip.totalPayout.toLocaleString("th-TH")}</span>
            </div>
          )}
          {canCancel && (
            <div className="pt-2">
              <button
                ...
                disabled={cancelling}
                className="w-full py-2.5 rounded-xl bg-ui-status-error text-ui-text-inverse text-[13px] font-bold hover:opacity-90 transition-opacity disabled:opacity-60"
              >

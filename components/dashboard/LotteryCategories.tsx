...
export default function LotteryCategories({ initialCategories = [], locale }: LotteryCategoriesProps) {
  ...
  return (
    <section className="space-y-4">
      <h2 className="text-[18px] font-bold text-text-strong tracking-tight flex items-center gap-2">
      ...
      {categories.map((cat: Category) => (
        <div key={cat.id} className="rounded-2xl border border-sky-100 shadow-card hover:shadow-card-hover transition-all overflow-hidden bg-white">
          ...
          <div className="relative bg-gradient-to-r from-[#0468ce] via-[#0a7bde] to-[#20a4ff] px-4 py-2.5 flex items-center gap-2 overflow-hidden">
            ...
            <span className="ml-auto relative bg-white/95 text-brand-primary text-[13px] font-bold rounded-full px-3 py-1 border border-white shadow-sm">
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <div className="min-w-0 flex-1">
                    <span className="block text-[15px] font-bold text-text-strong truncate leading-tight">{item.name}</span>
                    <span className="block text-[12px] text-text-muted truncate mt-0.5">

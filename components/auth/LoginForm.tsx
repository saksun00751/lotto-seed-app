...
function SubmitButton({ children, disabled }: { children: React.ReactNode; disabled?: boolean }) {
  ...
  return (
    <button
      ...
      className="w-full bg-ui-button-primary text-ui-text-inverse rounded-full py-3.5 text-[15px] font-semibold disabled:opacity-40 hover:bg-ui-button-primary-hover transition-all active:scale-[0.98] flex items-center justify-center gap-2"
    >
      {pending ? (
        <>
...
function ErrorBanner({ msg }: { msg: string }) {
  return (
    <div className="flex items-start gap-2.5 bg-ui-status-error/5 border border-ui-status-error-border/20 rounded-2xl px-4 py-3 animate-fade-in">
      ...
      <p className="text-[13px] text-ui-status-error">{msg}</p>
    </div>
  );
}
  ...
  if (success) {
    return (
      <div className="text-center py-4 animate-fade-up">
        ...
        <h2 className="text-[22px] font-bold text-ui-text">{t.successTitle}</h2>
        <p className="text-[14px] text-ui-text-soft mt-1.5">
          {t.successBack}{" "}
          <span className="font-semibold text-ui-text">{username || "-"}</span>
        </p>
        <div className="mt-5 flex items-center justify-center gap-2 text-[13px] text-ui-text-muted">
        ...
        <a href={`/${lang}/dashboard`} className="mt-2 block text-[13px] text-ui-status-info font-medium hover:opacity-70">{t.goNow}</a>
      </div>
    );
  }
          ...
          <label className="flex items-center gap-2 cursor-pointer group">
            <button type="button" onClick={() => setRemember(!remember)}
              className={["w-5 h-5 rounded-md border-2 flex items-center justify-center transition-all duration-200",
                remember ? "bg-ui-button-primary border-ui-selected-border" : "bg-surface-card border-ui-border group-hover:border-ui-selected-border/40"].join(" ")}>
            ...
            <span className="text-[13px] text-ui-text-soft select-none">{t.remember}</span>
          </label>
          <a href={`/${lang}/contact-public`} className="text-[13px] text-ui-status-info font-medium hover:opacity-70">{t.forgot}</a>
        </div>

  ...
  return (
    <div className="bg-surface-card rounded-2xl border border-ui-border shadow-card overflow-hidden">
      {promo.filepic ? (
        ...
        <div className="w-full aspect-[16/7] bg-gradient-to-br from-ui-button-primary to-blue-400 flex items-center justify-center">
          <span className="text-[40px]">🎁</span>
        </div>
      )}

      <div className="p-4 space-y-3">
        <h2 className="text-[15px] font-bold text-ui-text leading-tight">{promo.name_th}</h2>

        {hasBonus && (
          <div className="grid grid-cols-3 gap-2">
            {bonusPercent > 0 && (
              <div className="bg-surface-subtle rounded-xl px-3 py-2 text-center">
                <p className="text-[18px] font-extrabold text-ui-status-info tabular-nums">{bonusPercent}%</p>
                <p className="text-[10px] text-ui-text-muted mt-0.5">โบนัส</p>
              </div>
            )}
            {bonusMax > 0 && (
              <div className="bg-surface-subtle rounded-xl px-3 py-2 text-center">
                ...
                <p className="text-[10px] text-ui-text-muted mt-0.5">สูงสุด</p>
              </div>
            )}
            {turnpro > 0 && (
              <div className="bg-surface-subtle rounded-xl px-3 py-2 text-center">
                <p className="text-[15px] font-bold text-ui-text tabular-nums">{turnpro}x</p>
                <p className="text-[10px] text-ui-text-muted mt-0.5">เทิร์น</p>
              </div>
            )}
          </div>
        )}
        ...
        {promo.content && (
          <div
            className="text-[12px] text-ui-text-soft leading-relaxed prose prose-sm max-w-none"
  ...
  return (
    <div className="min-h-screen bg-surface-subtle pb-20 sm:pb-8">
      <div className="max-w-5xl mx-auto px-5 pt-6 space-y-5">
        <div>
          <h1 className="text-[22px] font-bold text-ui-text tracking-tight">โปรโมชั่น</h1>
          <p className="text-[13px] text-ui-text-soft mt-0.5">{sortedPromotions.length} โปรโมชั่น</p>
        </div>

        {sortedPromotions.length === 0 ? (
          <div className="bg-surface-card rounded-2xl border border-ui-border shadow-card py-14 flex flex-col items-center gap-2">
            <span className="text-[48px]">🎁</span>
            <p className="text-[13px] text-ui-text-muted">ยังไม่มีโปรโมชั่น</p>
          </div>
        ) : (
        ...
        {activePromotion && (
          <div className="bg-surface-card rounded-2xl border border-ui-border shadow-card px-5 py-4">

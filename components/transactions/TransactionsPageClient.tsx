...
function Empty({ label, suffix }: { label: string; suffix: string }) {
  return (
    <div className="py-16 flex flex-col items-center gap-2 text-center">
      ...
      <p className="text-[16px] font-bold text-text-strong">{label}</p>
      <p className="text-[14px] font-semibold text-text-muted">{suffix}</p>
    </div>
  );
}
...
function TxCard({ tx, t, locale }: { tx: TxRow; t: TT; locale: string }) {
  ...
  return (
    <div className="flex items-start justify-between px-4 py-3.5 border-b border-border-default last:border-0">
      <div className="min-w-0 flex-1">
        <div className="flex items-center gap-1.5 flex-wrap">
          <p className="text-[15px] font-bold text-text-strong">{tx.title}</p>
          <DirectionBadge direction={tx.direction} t={t} />
        </div>
        {tx.detail && (
          <p className="text-[13px] font-semibold text-text-default mt-0.5 leading-relaxed">{tx.detail}</p>
        )}
        <p className="text-[13px] font-semibold text-text-muted mt-0.5">{tx.date}</p>
      ...
      <div className="text-right flex-shrink-0 pl-4">
        ...
        {tx.balanceAfter > 0 && (
          <p className="text-[13px] font-semibold text-text-muted tabular-nums mt-0.5">
...
function SummaryBar({ summary, t, locale }: { summary: TxSummary; t: TT; locale: string }) {
  ...
  return (
    <div className="bg-white rounded-2xl border border-border-default shadow-card px-4 py-3 grid grid-cols-2 sm:grid-cols-4 gap-3">
      <div className="text-center">
        <p className="text-[13px] text-text-muted font-bold">{t.summaryTotal}</p>
        <p className="text-[17px] font-bold text-text-strong tabular-nums">{summary.count}</p>
      </div>
      <div className="text-center">
        <p className="text-[13px] text-text-muted font-bold">{t.summaryCredit}</p>
        <p className="text-[17px] font-bold text-emerald-600 tabular-nums">+{formatNumber(summary.totalCredit, locale)}</p>
      </div>
      <div className="text-center">
        <p className="text-[13px] text-text-muted font-bold">{t.summaryDebit}</p>
        <p className="text-[17px] font-bold text-red-500 tabular-nums">-{formatNumber(summary.totalDebit, locale)}</p>
      </div>
      <div className="text-center">
        <p className="text-[13px] text-text-muted font-bold">{t.summaryNet}</p>
    ...
    <div className="min-h-screen bg-surface-subtle pb-20 sm:pb-8">
      <div className="max-w-5xl mx-auto px-4 pt-5 space-y-4">
        ...
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-[22px] font-bold text-text-strong leading-tight">{t.title}</h1>
        </div>
            ...
            <button key={id} type="button"
              ...
              className={[
                ...
                id === tabId
                  ...
                  : "bg-white text-text-default border-border-default hover:bg-surface-subtle",
              ].join(" ")}
            >
        ...
        {/* Filter */}
        <form onSubmit={onSubmitFilter} className="bg-white rounded-2xl border border-border-default shadow-card p-3">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
            <input type="date" value={dateStartInput} onChange={(e) => setDateStartInput(e.target.value)}
              className="w-full border border-border-default rounded-xl px-3 py-2 text-[15px] font-semibold text-text-strong outline-none focus:border-brand-primary" />
            <input type="date" value={dateStopInput} onChange={(e) => setDateStopInput(e.target.value)}
              className="w-full border border-border-default rounded-xl px-3 py-2 text-[15px] font-semibold text-text-strong outline-none focus:border-brand-primary" />
          </div>
          <div className="mt-2 flex gap-2 justify-center">
            <button type="submit"
              className="px-4 py-2 rounded-xl bg-brand-primary text-white text-[15px] font-bold hover:bg-brand-primary-hover transition-colors">
              {t.search}
            </button>
            <button type="button" onClick={onClearFilter}
              className="px-4 py-2 rounded-xl border border-border-default text-[15px] font-bold text-text-default hover:bg-surface-subtle transition-colors">
              {t.clearDate}
            </button>
          </div>
        </form>
        ...
        <div className="bg-white rounded-2xl border border-border-default shadow-card overflow-hidden">
          <div className="px-4 py-3 border-b border-border-default flex items-center justify-between bg-slate-100/70">
            <div className="flex items-center gap-2">
              <span className="text-[20px] emoji-font">{TAB_ICONS[tabId]}</span>
              <span className="text-[16px] font-bold text-text-strong">{TAB_LABELS[tabId]}</span>
            </div>
          ...
          {loading && (
            <div className="py-16 flex items-center justify-center gap-2 text-text-default text-[15px] font-semibold">
              <span className="w-4 h-4 rounded-full border-2 border-brand-primary border-t-transparent animate-spin inline-block" />
              {t.loading}
            </div>
          )}

          {!loading && error && (
            <div className="px-4 py-6 text-[15px] font-semibold text-status-error">{error}</div>
          )}
          ...
          {!loading && !error && pagination.total > pagination.limit && (
            <div className="px-4 py-3 border-t border-border-default flex items-center justify-center gap-2">
              <button type="button" onClick={() => setPage((p) => Math.max(1, p - 1))}
                disabled={page <= 1}
                className="px-3 py-1.5 rounded-lg border border-border-default text-[14px] font-bold text-text-default disabled:opacity-40">
                {t.pagePrev}
              </button>
              <span className="text-[14px] font-bold text-text-default tabular-nums">

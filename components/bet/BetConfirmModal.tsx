  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className={"relative w-full max-w-md bg-surface-card rounded-3xl shadow-card-xl overflow-hidden flex flex-col max-h-[90dvh]"}>
        ...
        {/* Header */}
        <div className="px-5 py-4 border-b border-ui-border flex items-center justify-between shrink-0">
          <div>
            <p className="text-[16px] font-bold text-ui-text">{t.confirmTitle}</p>
            <p className="text-[14px] text-ui-text font-medium mt-0.5">{lotteryName} • {today}</p>
          </div>
          <button onClick={onCancel}
            className="w-8 h-8 flex items-center justify-center rounded-full bg-surface-subtle hover:bg-ui-border text-ui-text-soft transition-colors text-[18px]">
            ×
          </button>
        </div>
        ...
        <div className="overflow-y-auto flex-1">
          {groupMeta.map((group) => {
            ...
            return (
              <div key={group.key} className="border-b border-ui-border last:border-0">
                ...
                {items.map((b) => {
                  ...
                  return (
                    <div key={b.id} className="px-5 py-2.5 flex items-center gap-3 border-t border-ui-border first:border-t-0">
                      <div className="w-10 h-10 rounded-xl bg-ui-text flex items-center justify-center shrink-0">
                        <span className="text-ui-text-inverse font-extrabold text-[14px] tabular-nums">{b.number}</span>
                      </div>
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="mt-0.5 text-[14px] font-bold tabular-nums text-ui-status-info">
                          <span className="font-semibold text-[14px]">{getAmountLabel(b.betType, "top", t as Record<string, string>)} </span>
                          {amt}
                        </p>
                      </div>
                      <div className="text-right shrink-0">
                        <p className="text-[14px] font-bold text-ui-text tabular-nums">฿{amt.toLocaleString(numberLocale)}</p>
                      </div>
                    </div>
                  );
                })}
        ...
        {(() => {
          ...
          return (
            <div className="shrink-0 border-t border-ui-border bg-surface-subtle/70 px-5 py-3 space-y-1.5">
              ...
              <div className="flex items-center justify-between">
                <span className="text-[14px] font-semibold text-ui-text">ยอดก่อนหักส่วนลด</span>
              ...
              <div className="flex items-center justify-between pt-1.5 border-t border-ui-border">
                <span className="text-[15px] font-bold text-ui-text">ยอดหลังหักส่วนลด</span>
                <span className="text-[20px] font-bold text-ui-text tabular-nums">
        ...
        <div className="shrink-0 px-5 pb-5 pt-3 flex gap-3">
          <button onClick={onCancel}
            className="flex-1 py-3 rounded-2xl border-2 border-ui-border text-[14px] font-bold text-ui-text-soft hover:bg-surface-subtle active:scale-[0.98] transition-all">
            {t.cancel}
          </button>
          <button onClick={handleConfirm}
            className="flex-[2] py-3 rounded-2xl bg-ui-button-primary hover:bg-ui-button-primary-hover text-ui-text-inverse text-[14px] font-bold active:scale-[0.98] transition-all shadow-md">
            {t.save}
          </button>
        </div>

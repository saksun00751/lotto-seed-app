...
export default function BetQuickForm({
  ...
  return (
    ...
    <div className="bg-white rounded-2xl overflow-hidden shadow-card border border-border-default">
        ...
        {TABS.map((tab, i) => (
          <button key={tab.id}
            ...
            className={["flex-1 py-2.5 text-[14px] sm:text-[14px] font-bold transition-all",
              i < TABS.length - 1 ? "border-r border-border-default" : "",
              tab.disabled
                ? "bg-surface-subtle text-text-muted cursor-not-allowed"
                : activeTab === tab.id
                  ? "bg-brand-primary text-white"
                  : "bg-surface-subtle text-text-default hover:bg-white hover:text-text-strong",
            ].join(" ")}>
            {tabLabels[tab.id]}
          </button>
        ))}
      ...
      {activeTab !== "classic" && activeTab !== "standard" && <div className="p-4">
        {/* Title */}
        <div className="mb-3">
          <p className="text-[16px] font-bold text-text-strong">{tabLabels[activeTab]}</p>
        ...
        {/* Input area */}
        {activeTab === "slip" ? (
          <div className={["bg-surface-subtle/70 rounded-2xl border-2 border-brand-primary p-4 transition-opacity", hasSelection ? "" : "opacity-50 pointer-events-none"].join(" ")}>
            ...
            <textarea
              ...
              rows={4}
              className="w-full border-2 border-brand-primary rounded-xl px-3 py-3 text-[15px] font-bold text-text-strong outline-none focus:border-brand-primary focus:ring-4 focus:ring-brand-primary/15 bg-white shadow-sm transition-all resize-none leading-relaxed"
            />
            <p className="mt-1.5 text-[14px] text-text-default font-medium">{t.pasteSlipHint}</p>
          </div>
        ) : (
          <div className={["bg-surface-subtle/70 rounded-2xl border-2 border-brand-primary p-4 transition-opacity", hasSelection ? "" : "opacity-50 pointer-events-none"].join(" ")}>
            <label className="text-[14px] text-text-strong font-bold mb-1 block uppercase tracking-wide">
            ...
            <input
              ...
              className="w-full border-2 border-brand-primary rounded-xl px-3 py-3 text-[22px] text-center font-extrabold text-text-strong outline-none focus:border-brand-primary focus:ring-4 focus:ring-brand-primary/15 bg-white shadow-sm transition-all placeholder:text-text-muted/40"
            />
          </div>
        )}
      </div>}

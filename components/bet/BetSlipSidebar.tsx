...
export default function BetSlipSidebar({
  ...
  return (
    ...
    <div className="space-y-3">
      <div className="bg-white rounded-2xl overflow-hidden shadow-card border border-border-default sticky top-4">

        {/* Header */}
        <div className="px-4 py-3 border-b border-border-default bg-white">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-1.5">
              <span className="text-[17px] leading-none">📋</span>
              <span className="text-[15px] leading-none font-bold text-text-strong">{t.slipTitle}</span>
            </div>
            <div className="flex items-center gap-2">
              <span className="text-[14px] font-bold text-white bg-brand-primary px-2.5 py-0.5 rounded-full leading-none">
              ...
              {bills.length > 0 && (
                <button onClick={() => {
                  ...
                  setBulkAmount("");
                  onClearAll();
                }} className="text-[14px] font-semibold text-status-error hover:underline">
                  {t.clearAll}
                </button>
              )}
            </div>
          ...
          <div className="flex items-center gap-1.5 mt-2">
            ...
            <span className="text-[14px] text-text-strong font-semibold">{lotteryName}</span>
          </div>
        </div>
        ...
        {bills.length === 0 ? (
          <div className="py-14 flex flex-col items-center gap-2">
            ...
            <p className="text-[14px] font-bold text-text-strong">{t.noItems}</p>
            <p className="text-[14px] text-text-default font-medium">{t.enterAndAdd}</p>
          </div>
        ) : (
          <div className="divide-y divide-border-default max-h-[480px] overflow-y-auto">
            {groupMeta.map((group) => {
              const items = groupedBills[group.key];
              if (!items.length) return null;
              return (
                <div key={group.key}>
                  <div className="px-4 py-2 bg-surface-subtle/80 border-b border-border-default flex items-center justify-between">
                  ...
                  {items.map((b) => {
                    ...
                    return (
                      <div key={b.id} className="px-4 py-3 flex items-center gap-3 hover:bg-surface-subtle/60 transition-colors border-b border-border-default last:border-b-0">
                        <div className="w-12 h-12 rounded-2xl bg-text-strong flex items-center justify-center flex-shrink-0">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="flex items-center gap-1.5">
                            <input
                              ...
                              placeholder="0"
                              className="w-20 border-2 border-brand-primary/30 rounded-lg px-2 py-1 text-[14px] text-center font-extrabold text-brand-primary outline-none focus:border-brand-primary focus:ring-2 focus:ring-brand-primary/15 bg-blue-50/40 tabular-nums"
                            />
                          </div>
                        </div>
                        <div className="text-right flex-shrink-0 flex flex-col items-end">
                          <button onClick={() => onDelete(b.id)}
                            className="text-[14px] text-status-error hover:underline mb-1.5">
                            {t.delete}
                          </button>
                          <p className="text-[14px] text-text-default font-medium">ยอดชนะ</p>
                          <p className="text-[14px] font-bold text-status-success tabular-nums">
        ...
        {/* Bulk amount */}
        {bills.length > 0 && (
          <div className="px-4 py-3 border-t border-border-default bg-surface-subtle/40">
            <label className="text-[14px] text-text-strong font-bold mb-1.5 block uppercase tracking-wide">
              ใส่ยอดทุกบิล
            </label>
            <input
              type="text"
              ...
              placeholder="ยอด"
              className="w-full border-2 border-brand-primary/30 rounded-xl px-3 py-2.5 text-[15px] text-center font-extrabold text-brand-primary outline-none focus:border-brand-primary focus:ring-2 focus:ring-brand-primary/15 bg-white tabular-nums"
            />
            <div className="grid grid-cols-4 gap-1.5 mt-2">
              {[10, 20, 50, 100].map((v) => (
                <button
                  ...
                  className="py-2 rounded-lg bg-white border-2 border-border-default text-[14px] font-bold text-text-strong hover:border-brand-primary hover:bg-blue-50 active:scale-95 active:bg-brand-primary active:text-white transition-all tabular-nums"
                >
                  ฿{v}
                </button>
              ))}
        ...
        {bills.length > 0 && (
          <div className="border-t border-border-default">
            <div className="px-4 py-3 space-y-1.5">
              ...
              <div className="flex items-center justify-between">
                <span className="text-[16px] font-semibold text-text-strong">ยอดก่อนหักส่วนลด</span>
                <span className="text-[22px] font-bold text-text-strong tabular-nums">
              ...
              <div className="flex items-center justify-between pt-1 border-t border-border-default">
                <span className="text-[16px] font-bold text-text-strong">ยอดหลังหักส่วนลด</span>
                <span className="text-[22px] font-bold text-text-strong tabular-nums">
                  ฿{netAmount.toLocaleString(numberLocale, { maximumFractionDigits: 2 })}
                </span>
              </div>
              <div className="flex items-center justify-between pt-1 border-t border-border-default">
                <span className="text-[16px] text-status-error font-bold">⏱ {t.closeIn}</span>
                {closeAt
                  ? <CountdownTimer closeAt={closeAt} className="text-[16px] font-bold text-status-error tabular-nums" />
                  : <span className="text-[16px] font-bold text-status-error tabular-nums">—</span>
                }
              </div>
            </div>
            <div className="px-4 pb-4 space-y-2">
              <button onClick={handleOpenModal}
                className={[
                  ...
                  expired ? "bg-status-error/80 hover:bg-status-error" : "bg-brand-primary hover:bg-brand-primary-hover",
                ].join(" ")}>
    ...
    {saving && (
      <div className="fixed inset-0 z-50 bg-white/90 backdrop-blur-sm flex flex-col items-center justify-center gap-4">
        ...
        <div className="text-center">
          <p className="text-[15px] font-bold text-text-strong">{t.saving ?? "กำลังบันทึก..."}</p>
          <p className="text-[14px] text-text-default font-medium mt-1">{(t as Record<string,string>).pleaseWait ?? "กรุณารอสักครู่"}</p>
        </div>

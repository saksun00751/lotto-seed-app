...
export default function YeekeeShootsList({ roundId, autoRefresh = true }: { roundId: number; autoRefresh?: boolean }) {
  ...
  return (
    <section className="rounded-2xl border border-ui-border bg-surface-card shadow-card overflow-hidden flex flex-col">
      <div className="px-4 py-3 bg-gradient-to-r from-violet-600 to-fuchsia-500 border-b border-ui-border">
        <h2 className="text-[15px] font-extrabold text-ui-text-inverse">รายการผู้ทายเลข</h2>
        <p className="text-[12px] text-ui-text-inverse/80 font-medium">ทั้งหมด {count} รายการ</p>
      </div>

      <div className="flex-1 min-h-0 max-h-[420px] overflow-auto">
        {error && (
          <div className="p-4 text-[14px] text-ui-status-error font-semibold">{error}</div>
        )}
        {!error && loading && items.length === 0 && (
          <ShootsListSkeleton />
        )}
        {!error && items.length === 0 && !loading && (
          <div className="p-6 text-center text-[14px] text-ui-text-soft">ยังไม่มีรายการยิงเลข</div>
        )}
            ...
            {items.map((it, idx) => {
              ...
              return (
                ...
                >
                  ...
                  <span className="shrink-0 text-[12px] text-ui-text-muted tabular-nums">
      ...
      {!error && hasMore && (
        <div className="p-3 border-t border-ui-border bg-slate-50/60">
          <button
            type="button"
            onClick={loadMore}

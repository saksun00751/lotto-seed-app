...
const stats = useMemo(() => {
  ...
  const selectedInfo = useMemo(() => {
    ...
    let badgeCls = "bg-surface-subtle text-ui-text-soft";
    if (isLive) { label = t.yeekeeBadgeOpen; badgeCls = "bg-emerald-100 text-ui-status-success"; }
    else if (isFinished) {
      label = selectedRound.status === "voided" ? t.yeekeeBadgeVoided : t.yeekeeBadgeClosed;
      badgeCls = "bg-surface-subtle text-ui-text-muted";
    }
  ...
  return (
    <section className="bg-surface-card rounded-2xl border border-ui-border shadow-card overflow-hidden">
      <div className="bg-gradient-to-r from-violet-600 to-fuchsia-500 px-4 py-3 flex items-center gap-3">
        ...
        <div className="hidden sm:flex items-center gap-2 text-[14px] font-semibold text-ui-text-inverse/90">
      ...
      <div className="p-3 space-y-3">
        {loading && <div className="h-[92px] flex items-center justify-center text-[13px] text-ui-text-muted">{t.yeekeeLoadingRounds}</div>}
        {error && !loading && <div className="h-[92px] flex items-center justify-center text-[13px] text-ui-status-error">{error}</div>}
        ...
        {!loading && !error && rounds && (
          <div className="grid grid-cols-[repeat(auto-fill,minmax(2.75rem,1fr))] gap-2">
            {rounds.map((r) => {
              ...
              let stateCls = "";
              if (isCurrent && isLive) {
                stateCls = "bg-ui-status-success border-emerald-600 text-ui-text-inverse shadow-md";
              } else if (isLive) {
                stateCls = "border-emerald-300 text-ui-status-success hover:bg-emerald-50";
              } else if (isUpcoming) {
                stateCls = "border-transparent text-ui-text-soft hover:border-ui-selected-border/40";
              } else if (isResulted) {
                stateCls = "border-violet-400 bg-violet-50 text-violet-700 hover:bg-violet-100";
              } else {
                stateCls = "border-transparent text-ui-text-muted opacity-50 hover:opacity-80";
              }
        ...
        {selectedRound && selectedInfo && popupOpen && (
          ...
          >
            <div className="bg-surface-card w-full sm:max-w-md rounded-t-3xl sm:rounded-2xl overflow-hidden shadow-2xl max-h-[90vh] flex flex-col">
              <div className="relative flex items-center justify-center px-4 pt-4 pb-2 flex-shrink-0 sm:justify-end">
                <div className="w-10 h-1 bg-ui-border rounded-full sm:hidden" />
                <button
                  type="button"
                  onClick={closeSheet}
                  className="absolute right-4 top-2.5 flex w-9 h-9 rounded-full bg-surface-subtle items-center justify-center text-ui-text-soft hover:bg-ui-border/40 transition-colors sm:hidden"
                  aria-label="close"
                >
                ...
                <button
                  type="button"
                  onClick={closeSheet}
                  className="hidden sm:flex w-8 h-8 rounded-full bg-surface-subtle items-center justify-center text-ui-text-soft hover:bg-ui-border/40 transition-colors ml-auto"
                  aria-label="close"
                >
                  <svg className="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5">
                    <path d="M18 6L6 18M6 6l12 12" strokeLinecap="round" />
          ...
          {sheetView === "detail" ? (
          <div className="rounded-xl border border-ui-border bg-gradient-to-br from-violet-50 to-fuchsia-50 px-3 py-3 flex flex-col items-center gap-2">
            <div className="w-full flex items-center justify-between gap-2 flex-wrap">
              <div className="flex items-center gap-2">
                <span className="text-[18px] font-bold text-ui-text">{t.yeekeeRoundFmt.replace("{n}", String(selectedRound.round_no))}</span>
                <span className={`text-[13px] font-semibold rounded-full px-2 py-0.5 ${selectedInfo.badgeCls}`}>
                  {selectedInfo.label}
                </span>
              </div>
              <span className="text-[18px] font-bold text-ui-text tabular-nums">
            ...
            {selectedRound.status !== "resulted" && (
              <div className="text-center">
                <div className="text-[14px] font-semibold text-ui-text-muted leading-none">
                ...
                <div className={`text-[28px] font-extrabold tabular-nums leading-none mt-1 ${
                  selectedInfo.isLive ? "text-ui-status-success" : selectedInfo.isFinished ? "text-ui-text-muted" : "text-violet-600"
                }`}>
            ...
            {selectedInfo.isFinished && selectedRound.status !== "voided" && (() => {
              ...
              if (r === "loading") {
                return <div className="text-[13px] text-ui-text-muted">{t.yeekeeLoadingResult}</div>;
              }
              if (!r) return null;
              if ("error" in r) {
                return <div className="text-[13px] text-ui-status-error">{r.error}</div>;
              }
              return (
                <div className="w-full space-y-2">
                  <div className="grid grid-cols-2 gap-2">
                    <div className="flex flex-col items-center justify-center rounded-xl bg-surface-card border border-ui-border px-3 py-2">
                      <span className="text-[26px] font-extrabold tabular-nums text-ui-status-success leading-none">
                        {r.top_3 || "—"}
                      </span>
                  ...
                  <Link
                    ...
                    className="block w-full max-w-[240px] mx-auto text-center rounded-full bg-gradient-to-r from-violet-600 to-fuchsia-500 text-ui-text-inverse text-[14px] font-bold px-6 py-2.5 shadow-sm hover:opacity-90 active:scale-[0.98] transition-all"
                  >
                    {t.yeekeeCheckResult}
                  </Link>
                </div>
            ...
            {selectedInfo.isLive ? (
              <div className="w-full max-w-[200px]">
                <button
                  ...
                  className="inline-flex h-[34px] items-center justify-center gap-1.5 whitespace-nowrap w-full bg-ui-button-primary text-ui-text-inverse rounded-full px-4 text-[14px] font-bold border border-sky-200/50 hover:brightness-105 transition-all active:scale-95 disabled:opacity-70 shadow-md"
                >
                  {packagesLoading && (
              ...
              <button
                type="button"
                disabled
                className="max-w-[200px] flex items-center justify-center gap-2 text-[14px] font-bold text-ui-text-muted border border-ui-border rounded-full px-6 py-2.5 cursor-not-allowed"
              >
            ...
            <div className="space-y-3">
              <div className="flex items-center gap-2">
                <button
                  ...
                  className="w-9 h-9 rounded-full bg-surface-subtle border border-ui-border flex items-center justify-center text-ui-text-soft active:scale-95"
                  aria-label="back"
                >
                  <svg className="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5">
                ...
                <div className="min-w-0">
                  <h3 className="text-[16px] font-extrabold text-ui-text leading-tight">{t.selectPackage}</h3>
                  <p className="text-[12px] font-semibold text-ui-text-muted">
              ...
              {packageError && (
                <div className="rounded-xl border border-ui-status-error-border/20 bg-ui-status-error/5 px-3 py-2 text-[13px] font-semibold text-ui-status-error">
                  {packageError}
                </div>
              )}

              {packagesLoading ? (
                <div className="h-28 flex items-center justify-center text-[13px] font-semibold text-ui-text-muted">
                ...
                <div className="flex flex-col gap-3">
                  {packages.map((pkg) => (
                    <button
                      ...
                      className="group w-full overflow-hidden rounded-2xl border border-ui-border bg-surface-card text-left shadow-sm transition-all active:scale-[0.98] disabled:opacity-70"
                    >
                      {resolveImage(pkg.image) ? (
                        ...
                        <div className="flex min-h-24 items-center justify-center bg-surface-subtle px-4 py-5 text-center text-[15px] font-bold text-ui-text">
                          {pkg.name}
                        </div>
                      )}
                    </button>

...
export default function BetClassicForm({ lotteryFlag, lotteryLogo, bills, numberLimits, bettingContext, onAddBills }: Props) {
  ...
  // ── Render ─────────────────────────────────────────────────────────────────
  const inputCls = "w-full text-center text-[14px] font-extrabold text-ui-status-info outline-none bg-surface-card tabular-nums py-2 focus:bg-blue-50 focus:ring-2 focus:ring-inset focus:ring-ui-status-info/20 transition-all placeholder:text-ui-text-muted/50 placeholder:font-normal";
  ...
  return (
    <div className="p-0">
      ...
      {/* Toast */}
      {toast && (
        <div className="mx-4 mt-3 px-3 py-2.5 rounded-xl bg-yellow-50 border border-yellow-200 text-[14px] text-yellow-800 font-semibold flex items-center gap-2">
          <span>⚠️</span> {toast}
        </div>
      )}
      ...
      <div className="px-3 pb-2 pt-3 overflow-x-auto">
        <table className="w-full border-collapse text-[14px] rounded-xl overflow-hidden" style={{ minWidth: 340 }}>
          <thead>
            <tr className="bg-ui-button-primary text-ui-text-inverse">
              <th className="border border-ui-selected-border/40 px-2 py-2.5 text-center font-bold w-[28%]">
                <div className="text-[14px]">{t.numberLabel}</div>
                <button
                  onClick={handleReverse}
                  tabIndex={-1}
                  className="text-[14px] text-blue-200 underline mt-0.5 hover:text-ui-text-inverse transition-colors"
                >
                  ({t.reverseNumbers})
                </button>
              </th>
              <th className="border border-ui-selected-border/40 px-2 py-2.5 text-center font-bold w-[21%]">
                <div className="text-[14px]">{t.top}</div>
                {(ctx3top?.payout || ctx2top?.payout) && (
                  <div className="text-[14px] text-ui-status-success font-bold mt-0.5">
                    {ctx3top?.payout ? `×${ctx3top.payout}` : ""}{ctx3top?.payout && ctx2top?.payout ? "/" : ""}{ctx2top?.payout ? `×${ctx2top.payout}` : ""}
                  </div>
                )}
                <button onClick={() => handleCopyCol("top")} tabIndex={-1} className="text-[14px] text-blue-200 underline mt-0.5 hover:text-ui-text-inverse transition-colors">
                  ({t.copy})
                </button>
              </th>
              <th className="border border-ui-selected-border/40 px-2 py-2.5 text-center font-bold w-[21%]">
                <div className="text-[14px]">{t.bottom}</div>
                {ctx2bot?.payout ? <div className="text-[14px] text-ui-status-success font-bold mt-0.5">×{ctx2bot.payout}</div> : null}
                <button onClick={() => handleCopyCol("bot")} tabIndex={-1} className="text-[14px] text-blue-200 underline mt-0.5 hover:text-ui-text-inverse transition-colors">
                  ({t.copy})
                </button>
              </th>
              <th className="border border-ui-selected-border/40 px-2 py-2.5 text-center font-bold w-[21%]">
                <div className="text-[14px]">{t.tod}</div>
                {ctx3tod?.payout ? <div className="text-[14px] text-ui-status-success font-bold mt-0.5">×{ctx3tod.payout}</div> : null}
                <button onClick={() => handleCopyCol("tod")} tabIndex={-1} className="text-[14px] text-blue-200 underline mt-0.5 hover:text-ui-text-inverse transition-colors">
                  ({t.copy})
                </button>
              </th>
              <th className="border border-ui-selected-border/40 px-2 py-2.5 text-center font-bold w-[9%] text-[14px]">{t.delete}</th>
            </tr>
          </thead>
          <tbody>
            {rows.map((row, i) => {
              ...
              return (
                <tr key={row.id} className={i % 2 === 0 ? "bg-surface-card hover:bg-gray-200" : "bg-surface-subtle/40 hover:bg-surface-subtle"} style={{ transition: "background 0.1s" }}>
                  <td className="border border-ui-border px-2 py-1 text-center font-extrabold text-ui-status-info tabular-nums text-[14px]">
                    {row.number}
                  </td>
                  {/* บน */}
                  <td className="border border-ui-border p-0">
                    <input
                  ...
                  {/* ล่าง */}
                  <td className={`border border-ui-border p-0 ${!is2 ? "bg-gray-200" : ""}`}>
                    {is2
                      ? <input type="text" inputMode="numeric" value={row.bot}
                          ...
                          className={inputCls} />
                      : <div className="py-1.5 text-center text-ui-text-muted text-[14px]">—</div>
                    }
                  </td>
                  {/* โต๊ด */}
                  <td className={`border border-ui-border p-0 ${is2 ? "bg-gray-200" : ""}`}>
                  ...
                  <td className="border border-ui-border px-1 py-1 text-center">
                    <button
                      tabIndex={-1}
                      onClick={() => deleteRow(row.id)}
                      className="w-6 h-6 rounded-lg bg-ui-status-error/10 hover:bg-ui-status-error text-ui-status-error hover:text-ui-text-inverse transition-colors flex items-center justify-center mx-auto text-[14px]"
                    >
                      ✕
                    </button>
                  </td>
            ...
            {/* Input row */}
            <tr className="bg-ui-button-primary/5 border-t-2 border-ui-selected-border/20">
              <td className="border border-ui-border p-0">
                <input
                  ref={inputRef}
                  ...
                  maxLength={3}
                  className="w-full text-center text-[15px] font-extrabold text-ui-status-info outline-none bg-surface-card tabular-nums py-2.5 focus:bg-blue-50 focus:ring-2 focus:ring-inset focus:ring-ui-status-info/20 transition-all placeholder:text-ui-text-muted placeholder:font-normal placeholder:text-[14px]"
                />
              </td>
              <td className="border border-ui-border bg-surface-subtle/40" />
              <td className="border border-ui-border bg-surface-subtle/40" />
              <td className="border border-ui-border bg-surface-subtle/40" />
              <td className="border border-ui-border bg-surface-subtle/40" />
            </tr>
      ...
      <div className="px-4 pb-3 pt-1 flex items-center gap-2">
        <label className="text-[14px] text-ui-text-soft font-semibold whitespace-nowrap">{t.note}:</label>
        <input
          type="text" value={note} onChange={(e) => setNote(e.target.value)}
          placeholder={t.notePlaceholder}
          className="flex-1 border-2 border-ui-selected-border rounded-xl px-3 py-1.5 text-[14px] outline-none focus:ring-2 focus:ring-ui-status-info/10 bg-surface-card transition-all"
        />
        {lotteryLogo
      ...
      <div className="px-4 pb-4 flex gap-2">
        <button
          onClick={clearAll}
          className="flex-1 py-2.5 rounded-xl border border-ui-border text-[14px] font-semibold text-ui-text-soft hover:bg-surface-subtle active:scale-95 transition-all"
        >
          {t.clearAll}
        </button>
        <button
          ...
          className={[
            ...
            canSubmit
              ? "bg-ui-button-primary text-ui-text-inverse hover:bg-ui-button-primary-hover shadow-md"
              : "bg-surface-subtle border border-dashed border-ui-border text-ui-text-muted cursor-not-allowed",
          ].join(" ")}
        >
          + {t.addBill}
        </button>

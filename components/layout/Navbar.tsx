...
export default function Navbar({ logoUrl, balance, diamond, userName, userPhone, mobileNavItems }: NavbarProps) {
  ...
  return (
    <>
      <nav className="sticky top-0 z-50 bg-surface-navbar backdrop-blur-xl border-b border-ui-border">
        <div className="max-w-5xl mx-auto px-3 sm:px-5 h-16 flex items-center justify-between gap-2">
              ...
              return (
                <Link key={l.href} href={l.href}
                  className={[
                    ...
                    active ? "bg-ui-button-primary/10 text-ui-status-info" : "text-ui-text-soft hover:bg-surface-subtle hover:text-ui-text",
                  ].join(" ")}>
          ...
          <div className="flex items-center gap-1.5 sm:gap-2">
            ...
            <Link href={`/${lang}/deposit`}
              className="flex items-center gap-1.5 bg-surface-subtle border border-ui-border rounded-full px-2.5 sm:px-3 py-1.5 hover:border-ui-selected-border/30 transition-colors">
              <AppIcon name="deposit" className="text-[14px] leading-none" />
                <span className="text-[13px] sm:text-[13px] font-bold text-ui-text tabular-nums">
            ...
            <div className="relative" ref={dropRef}>
              <button
                ...
                className={[
                  "w-9 h-9 rounded-full bg-surface-card border border-ui-border flex items-center justify-center text-ui-text shadow-sm transition-all",
                  profileOpen ? "ring-2 ring-ui-status-info/25 border-ui-selected-border/35 shadow-md" : "hover:border-ui-selected-border/35 hover:shadow-md",
                ].join(" ")}
                ...
                <div className="absolute right-0 top-[calc(100%+8px)] w-[248px] bg-surface-card rounded-2xl shadow-[0_16px_36px_rgba(15,23,42,0.18)] border border-slate-200 overflow-x-hidden overflow-y-auto max-h-[calc(100dvh-88px)] sm:max-h-[80vh] overscroll-contain animate-pop-in z-50">
                  ...
                  <div className="px-4 pt-2.5 pb-2 flex items-center justify-between">
                    ...
                    <button
                      ...
                      className="flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-surface-card border border-slate-200 text-[13px] font-bold text-slate-700 hover:border-ui-selected-border/35 transition-all"
                    >
                      <img
                      ...
                      <svg className="w-3 h-3 text-ui-text-muted" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><path d="M19 9l-7 7-7-7"/></svg>
                    </button>
                  </div>
                  ...
                  <div className="px-4 pb-3">
                    <div className="flex items-center gap-3">
                      <div className="w-9 h-9 rounded-full bg-ui-button-primary flex items-center justify-center text-ui-text-inverse font-bold text-[14px] flex-shrink-0">
                        {initials}
                      </div>
                      <div className="min-w-0">
                        <p className="text-[15px] font-bold text-ui-text truncate">{liveUserName}</p>
                        {liveUserPhone && <p className="text-[13px] font-semibold text-ui-text-muted">{liveUserPhone}</p>}
                      </div>
                    </div>
                    <div className="mt-3 grid grid-cols-2 gap-1.5">
                      <div className="flex flex-col items-center rounded-xl px-2.5 py-2 border border-slate-200 bg-slate-50/70">
                        ...
                        <span className="text-[17px] leading-none mt-0.5 font-extrabold text-ui-status-info tabular-nums tracking-tight">
                          ฿{liveBalance.toLocaleString("th-TH", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                        </span>
                      </div>
                      <div className="flex flex-col items-center rounded-xl px-2.5 py-2 border border-slate-200 bg-slate-50/70">
                  ...
                  <div className="py-1.5">
                    {profileMenuItems.map((item) => {
                      ...
                      <Link
                        ...
                        className={[
                          ...
                          active
                            ? "bg-ui-button-primary/10 text-ui-status-info font-bold"
                            : "text-ui-text font-semibold hover:bg-ui-selected-bg hover:border-ui-selected-border hover:shadow-[0_6px_14px_rgba(37,99,235,0.10)] hover:-translate-y-[1px]",
                        ].join(" ")}
                      >
                        <span className={[
                          "w-6.5 h-6.5 rounded-lg flex items-center justify-center border transition-all duration-200",
                          active
                            ? "bg-surface-card border-ui-selected-border/25 text-ui-status-info"
                            : "bg-slate-50 border-slate-200 text-slate-600 group-hover:border-ui-selected-border group-hover:bg-ui-selected-bg group-hover:text-ui-status-info group-hover:scale-[1.04]",
                        ].join(" ")}>
                          <AppIcon name={item.icon} className="w-4 h-4" />
                        </span>
                        {item.label}
                        <svg className="ml-auto w-3.5 h-3.5 text-slate-400 transition-all duration-200 group-hover:text-ui-status-info group-hover:translate-x-0.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                    ...
                    <div className="border-t border-slate-200/80 pt-1.5 mt-1">
                      <form action={logoutAction}>
                        <button
                          type="submit"
                          className="group w-full flex items-center gap-2.5 px-3 py-2.5 text-[14px] font-bold transition-all duration-200 ease-out text-ui-status-error hover:bg-gradient-to-r hover:from-rose-50 hover:to-white hover:border-rose-200/80 border border-transparent hover:shadow-[0_6px_14px_rgba(244,63,94,0.10)] rounded-xl mx-1.5"
                        >
                          <span className="w-6.5 h-6.5 rounded-lg flex items-center justify-center border bg-rose-50 border-rose-200 text-ui-status-error transition-transform duration-200 group-hover:scale-[1.04]">
      ...
      {langModalOpen && (
        ...
        >
          <div
            className="bg-surface-card rounded-2xl shadow-card-xl border border-ui-border w-[280px] overflow-hidden animate-pop-in"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="px-5 py-4 border-b border-ui-border">
              <p className="text-[15px] font-bold text-ui-text">{t.language}</p>
            </div>
              ...
              {LANGS.map((l) => (
                <button
                  ...
                  className={[
                    ...
                    lang === l.code
                      ? "bg-ui-button-primary/10 text-ui-status-info border border-ui-selected-border/30"
                      : "text-ui-text hover:bg-surface-subtle border border-transparent",
                  ].join(" ")}
                >
                  <img src={l.flagIcon} alt={l.flag} className="w-5 h-5 rounded-sm object-cover" />
                  <span>{l.label}</span>
                  {lang === l.code && (
                    <svg className="ml-auto w-4 h-4 text-ui-status-info" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><path d="M5 13l4 4L19 7"/></svg>
                  )}
                </button>
              ))}
            </div>
      ...
      {/* Mobile bottom tabs */}
      <div
        className="sm:hidden fixed bottom-0 left-0 right-0 bg-surface-navbar backdrop-blur-xl border-t border-ui-border z-50 pb-[env(safe-area-inset-bottom)]"
      >
            ...
            return (
              <Link key={l.href} href={l.href}
                className={[
                  ...
                  active ? "text-ui-status-info" : "text-ui-text-muted",
                ].join(" ")}>
                {l.rawIcon ? (

...
function StatusBadge({ status }: { status: string }) {
  const map: Record<string, { cls: string; label: string }> = {
    pending_claim: { cls: "bg-amber-100 text-ap-orange border-amber-300",   label: "รอรับโบนัส" },
    claimed:       { cls: "bg-emerald-100 text-ap-green border-emerald-300", label: "รับแล้ว" },
    expired:       { cls: "bg-gray-100 text-gray-500 border-gray-300",      label: "หมดอายุ"  },
    used:          { cls: "bg-red-100 text-ap-red border-red-300",         label: "ใช้แล้ว"  },
  };
...
function TypeBadge({ type, label }: { type: string; label: string }) {
  const colors: Record<string, string> = {
    ...
    cashback: "bg-green-100 text-ap-green",
  };
  return (
...
function CouponCard({
  ...
  return (
    <div className={`relative bg-surface-card rounded-2xl border overflow-hidden shadow-card transition-all ${isActive ? "border-amber-300" : "border-ap-border opacity-80"}`}>
      ...
      <div className="px-4 pt-3 pb-4">
        ...
        <div className="flex items-start justify-between gap-2 mb-3">
          ...
          <div className="text-right flex-shrink-0">
            <p className="text-[26px] font-extrabold text-ap-orange tabular-nums leading-tight">
        ...
        <div className="border-t border-dashed border-ap-border my-3 relative">
          <div className="absolute -left-6 top-1/2 -translate-y-1/2 w-4 h-4 rounded-full bg-surface-subtle border border-ap-border" />
          <div className="absolute -right-6 top-1/2 -translate-y-1/2 w-4 h-4 rounded-full bg-surface-subtle border border-ap-border" />
        </div>
        ...
        {item.can_claim && !isClaimed ? (
          ...
          <div className="w-full py-2.5 rounded-xl bg-surface-subtle border border-ap-border text-center text-[15px] font-bold text-ap-tertiary">
            ✓ {item.status_label}
          </div>
        ) : null}
        ...
        {/* Toast */}
        {toast && (
          <div className={`mt-2 px-3 py-2 rounded-xl text-[14px] font-bold text-center ${toast.ok ? "bg-emerald-50 text-ap-green" : "bg-red-50 text-ap-red"}`}>
            {toast.msg}
          </div>
        )}
      </div>
...
function RedeemBox({ onRedeem }: { onRedeem: (code: string) => Promise<{ success: boolean; message?: string }> }) {
  ...
  return (
    <div className="bg-surface-card rounded-2xl border border-ap-border shadow-card p-4">
      ...
      <form onSubmit={handleRedeem} className="flex gap-2">
        <input
          ...
          maxLength={30}
          className="flex-1 border border-ap-border rounded-xl px-3 py-2.5 text-[15px] font-mono font-bold uppercase tracking-wider outline-none focus:border-ap-blue bg-surface-card transition-all"
        />
        <button
          type="submit"
      ...
      {toast && (
        <div className={`mt-2 px-3 py-2 rounded-xl text-[14px] font-bold ${toast.ok ? "bg-emerald-50 text-ap-green" : "bg-red-50 text-ap-red"}`}>
          {toast.msg}
        </div>
      )}
    </div>
  ...
  return (
    <div className="min-h-screen bg-surface-subtle pb-20 sm:pb-8">
      <div className="max-w-2xl mx-auto px-4 pt-5 space-y-4">
        ...
        {/* List */}
        {items.length === 0 ? (
          <div className="bg-surface-card rounded-2xl border border-ap-border shadow-card py-16 text-center">

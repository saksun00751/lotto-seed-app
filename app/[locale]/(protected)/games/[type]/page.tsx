...
function ProviderTile({ game, locale }: { game: GameProviderItem; locale: string }) {
  return (
    ...
    >
      ...
      <p className="text-[11px] font-semibold text-text-strong leading-tight line-clamp-2 w-full">{game.name}</p>
    </Link>
  );
}
      ...
      <div className="max-w-5xl mx-auto px-4 pt-5">
        ...
        <div className="bg-white rounded-2xl border border-border-default shadow-card overflow-hidden">
          ...
          <div className="flex items-center gap-3 px-4 py-3 border-b border-border-default">
            ...
            <div>
              <h1 className="text-[16px] font-bold text-text-strong leading-tight">{meta.emoji} {meta.label}</h1>
              <p className="text-[12px] text-text-default">{games.length} {t.providers}</p>
            </div>
          </div>
          {games.length === 0 ? (
            <div className="py-16 flex flex-col items-center gap-2">
              <span className="text-[48px]">{meta.emoji}</span>
              <p className="text-[13px] text-text-muted">{t.noProviders}</p>
            </div>
          ) : (
            isCardGroupPage ? (
              <div className="p-3 space-y-5">
                {groupedGames.map((group) => (
                  <section key={group.type} className="overflow-hidden rounded-2xl border border-border-default bg-white">
                    <div className="flex items-center gap-2 border-b border-border-default bg-surface-subtle px-3 py-2.5">
                      <div className="flex items-center gap-2 min-w-0">
                        <span className="text-[18px] shrink-0">{CARD_GROUP_EMOJI[group.type] ?? meta.emoji}</span>
                        <div className="min-w-0">
                          <h2 className="text-[14px] font-bold text-text-strong leading-tight truncate">{group.label}</h2>
                          <p className="text-[12px] text-text-default leading-tight mt-0.5">{group.games.length} {t.providers}</p>
                        </div>
                      </div>
                    </div>

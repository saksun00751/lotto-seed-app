...
function SubItemCard({ item, t, groupId, locale }: { item: SubItem; t: CategoryTranslation; groupId: number; locale: string }) {
  return (
    <div className="bg-white rounded-2xl border border-border-default shadow-card hover:shadow-card-hover transition-all hover:-translate-y-0.5 overflow-hidden h-full">
      ...
      <div className="p-4 h-full flex flex-col">
        ...
        <div className="flex items-start justify-between mb-2.5">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <div className="text-[14px] font-semibold text-text-strong leading-tight">{item.name}</div>
              <div className="text-[14px] font-bold text-status-error mt-0.5 tabular-nums">{item.sub}</div>
            </div>
          </div>
          {item.isOpen ? (
            <span className="flex items-center gap-1 text-[12px] font-medium text-status-error bg-status-error/8 px-2 py-0.5 rounded-full border border-status-error/15 flex-shrink-0">
              <span className="w-1.5 h-1.5 rounded-full bg-status-error animate-pulse inline-block" />
              {t.live}
            </span>
          ) : (
            <span className="text-[12px] font-medium text-status-success bg-status-success/8 px-2 py-0.5 rounded-full border border-status-success/15 flex-shrink-0">
              ✓ {t.drawn}
            </span>
          )}
        </div>
        ...
        {item.result ? (
          <div className="grid grid-cols-2 gap-1.5 h-[68px] mb-3">
            <div className="bg-surface-subtle rounded-xl p-2 text-center flex flex-col justify-center">
              <div className="text-[20px] font-bold text-text-strong tabular-nums">{item.result.top3}</div>
              <div className="text-[12px] text-text-muted">{t.top3}</div>
            </div>
            <div className="bg-surface-subtle rounded-xl p-2 text-center flex flex-col justify-center">
              <div className="text-[20px] font-bold text-text-strong tabular-nums">{item.result.bot2}</div>
              <div className="text-[12px] text-text-muted">{t.bot2}</div>
            </div>
          </div>
        ) : (
          <div className="text-center h-[68px] flex flex-col items-center justify-center mb-3">
            {item.closeAt
              ? <CountdownTimer closeAt={item.closeAt} className="text-[22px] font-bold text-text-strong tabular-nums tracking-wide" expiredText="—" />
              : <span className="text-[22px] font-bold text-text-strong tabular-nums tracking-wide">{item.countdown ?? "—"}</span>
            }
          </div>
        )}
        ...
        <div className="mt-auto">
          {item.isOpen ? (
            ...
            <button className="block w-full text-center bg-surface-subtle border border-border-default text-text-default rounded-full py-2 text-[13px] font-medium cursor-default">
              {t.viewPast}
            </button>
          )}
        </div>
...
function ComingSoon({ emoji, label, t }: { emoji: string; label: string; t: CategoryTranslation }) {
  return (
    <div className="flex flex-col items-center justify-center py-20 text-center">
      ...
      <h2 className="text-[20px] font-bold text-text-strong mb-2">{label}</h2>
      <p className="text-[14px] text-text-muted">{t.comingSoon}</p>
    </div>
  );
}
    ...
    <div className="min-h-screen bg-surface-subtle pb-20 sm:pb-8">
      ...
      <div className="max-w-5xl mx-auto px-5 pt-6">
        ...
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            <h1 className="text-[18px] font-bold text-text-strong leading-tight">{cat.label}</h1>
            {openCount > 0 ? (
              <p className="text-[12px] text-status-error font-medium">🔴 {openCount} {t.liveItems}</p>
            ) : (
              <p className="text-[12px] text-text-muted">{cat.items.length} {t.items}</p>
            )}
          </div>
        </div>

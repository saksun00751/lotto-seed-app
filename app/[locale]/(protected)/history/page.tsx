    ...
    <div className="min-h-screen bg-surface-subtle pb-20 sm:pb-8">
      <div className="max-w-4xl mx-auto px-4 pt-5 space-y-4">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-[18px] font-bold text-ui-text leading-tight">{t.title}</h1>
            <p className="text-[12px] text-ui-text-soft">{t.found} {total.toLocaleString()} {t.items}</p>
          </div>
        </div>
            ...
            <Link key={tab.id} href={buildHref({ status: tab.id, page: 1 })}
              className={[
                ...
                status === tab.id
                  ...
                  : "bg-surface-card border border-ui-border text-ui-text-soft hover:bg-surface-subtle",
              ].join(" ")}>
              {tab.label}
            </Link>
          ))}
        ...
        <div className="bg-surface-card rounded-2xl border border-ui-border shadow-card overflow-hidden">
          {pageItems.length === 0 ? (
            <div className="py-16 text-center">
              ...
              <p className="text-[14px] font-semibold text-ui-text mb-1">{t.emptyTitle}</p>
              <p className="text-[12px] text-ui-text-soft">{t.emptyDesc}</p>
              <Link href={`/${locale}/bet`}
                className="inline-block mt-4 px-6 py-2.5 bg-ui-button-primary text-ui-text-inverse rounded-full text-[13px] font-semibold hover:bg-ui-button-primary-hover transition-colors">
                {t.gobet}
              </Link>
            </div>
          ) : (
          ...
          <div className="flex items-center justify-center gap-1 py-2 flex-wrap">
            {serverPage > 1 ? (
              <Link href={buildHref({ page: serverPage - 1 })}
                className="px-3 py-2 bg-surface-card border border-ui-border rounded-xl text-[13px] font-semibold text-ui-text-soft hover:bg-surface-subtle transition-colors">←</Link>
            ) : (
              <span className="px-3 py-2 rounded-xl text-[13px] text-ui-border select-none">←</span>
            )}
            {pageNums.map((n, i) =>
              n === "..." ? (
                <span key={`dots-${i}`} className="px-2 py-2 text-[13px] text-ui-text-soft">…</span>
              ) : (
                <Link key={n} href={buildHref({ page: n })}
                  className={[
                    ...
                    serverPage === n ? "bg-ui-button-primary text-ui-text-inverse" : "bg-surface-card border border-ui-border text-ui-text-soft hover:bg-surface-subtle",
                  ].join(" ")}>
                  {n}
                </Link>
              )
